...
namespace game
{
	...
	void PhysicsManager::ResolveBodyIntersect(CircleBody& body1, CircleBody& body2) const
	{
	...
	core::Vec2f PhysicsManager::ContactPoint(const CircleBody& body1, const CircleBody& body2)
	{
		const double ratio = (body1.radius) / ((body1.radius) + (body2.radius));
	...
	{
		const double ratio = (body.radius) / (body.position - v).Length();
		return (body.position - v) * ratio + v;
	}

	float PhysicsManager::CalculateDistance(CircleBody body1, CircleBody body2) const
	{
	...
	bool PhysicsManager::BodyIntersect(CircleBody body1, CircleBody body2) const
	{
